...
CREATE TABLE antivirus (
  ...
  scanner_timeout		 int NOT NULL DEFAULT  '300',
  silent			 enum('yes', 'no') DEFAULT 'yes',
  file_timeout			 int NOT NULL DEFAULT  '20',
  expand_tnef			 enum('yes', 'no') DEFAULT 'yes',
  deliver_bad_tnef		 enum('yes', 'no') DEFAULT 'no',
  tnef_timeout			 int NOT NULL DEFAULT  '120',
  usetnefcontent        enum('no', 'add', 'replace') DEFAULT 'no',
  max_message_size		 int NOT NULL DEFAULT  '0',
  max_attach_size		 int NOT NULL DEFAULT  '-1',
  max_archive_depth		 int NOT NULL DEFAULT  '0',
  max_attachments_per_message    int NOT NULL DEFAULT  200,
