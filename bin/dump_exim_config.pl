...
foreach my $stage (@eximids) {
    ...
    while ( $custom != 2) {
        my $dir;
        my $dest_dir = "$SRCDIR}/etc/exim/stage${stage}";
        if ($custom) {
...
sub ownership($stage)
{
    ...
    `usermod -a -G mailcleaner mailcleaner` unless (grep(/\bmailcleaner\b/, `groups mailcleaner`));
...
";
    ...
    if (-e "/etc/apparmor.d/usr.sbin.exim4") {
        `apparmor_parser -R /etc/apparmor.d/usr.sbin.exim4`;
        unlink("/etc/apparmor.d/usr.sbin.exim4");
    }
    `apparmor_parser -r ${SRCDIR}/etc/apparmor.d/opt.exim4.bin.exim` if ( -d '/sys/kernel/security/apparmor' );
