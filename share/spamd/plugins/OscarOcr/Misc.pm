...
use strict;
package OscarOcr::Misc;

use base 'Exporter';
our @EXPORT_OK = qw(max removedirs removedir save_execute);

use lib qw(..);
use OscarOcr::Config qw(set_pid unset_pid get_config get_timeout kill_pid);
...
sub open_on_specific_fd($$) {
  my($fd_target,$fname_tainted) = @_;
  my($flags) = 0; my($mode) = 0640;
  $fname_tainted =~ m/^(\S+)$/;
  my $fname = $1;
  $fname =~ s/^< *//;
  if ($fname =~ s/^>> *// ) { 
     $flags = POSIX::O_CREAT|POSIX::O_WRONLY|POSIX::O_APPEND;
  }
  if ($fname =~ s/^> *// ) {
     $flags = POSIX::O_CREAT|POSIX::O_WRONLY;
  }
...
sub save_execute {
    ...
    if ($conf->{'oscar_global_timeout'}) {
        ...
        if ($t->timed_out()) {
            errorlog("Command \"$cmd\" timed out after $conf->{oscar_timeout} seconds.");
