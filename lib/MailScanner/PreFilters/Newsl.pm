...
## analyze result
  ...
  if ($ret == 2) {
    MailScanner::Log::InfoLog("$MODULE (position ".$Newsl::conf{position}.": ".($Newsl::conf{pos_decisive})?'':'not '."decisive) result is newsletter ($score/$limit) for ".$message->{id});
    if ($Newsl::conf{'putHamHeader'}) {
      $global::MS->{mta}->AddHeaderToOriginal($message, $Newsl::conf{'header'}, $Newsl::conf{pos_text}."is newsletter ($score/$limit)");
    }
    $message->{prefilterreport} .= ", Newsl (position=".$Newsl::conf{position}.", decisive=".(($Newsl::conf{pos_decisive})?'newsletter':'false').", score=".$score.", required=".$limit.", ".$rulesum.")";
  ...
  MailScanner::Log::InfoLog("$MODULE (position ".$Newsl::conf{position}.": ".($Newsl::conf{neg_decisive})?'':'not '."decisive) result is not newsletter ($score/$limit) for ".$message->{id});
  if ($Newsl::conf{'putSpamHeader'}) {
    $global::MS->{mta}->AddHeaderToOriginal($message, $Newsl::conf{'header'}, $Newsl::conf{neg_text}."is not newsletter ($score/$limit)");
  }
  $message->{prefilterreport} .= ", Newsl (position=".$Newsl::conf{position}.", decisive=".(($Newsl::conf{neg_decisive})?'ham':'false').", score=".$score.", required=".$limit.", ".$rulesum.")";
  return 0;
}
