...
sub initialise {
  ...
  if ($MessageSniffer::conf{'pos_decisive'} && ($MessageSniffer::conf{'decisive_field'} eq 'pos_decisive' || $MessageSniffer::conf{'decisive_field'} eq 'both')) {
    $MessageSniffer::conf{'pos_decisive'} = '+'.$MessageSniffer::conf{'position'}.'+ ';
  } else {
    $MessageSniffer::conf{'pos_decisive'} = '~'.$MessageSniffer::conf{'position'}.'~ ';
  }
  if ($MessageSniffer::conf{'neg_decisive'} && ($MessageSniffer::conf{'decisive_field'} eq 'neg_decisive' || $MessageSniffer::conf{'decisive_field'} eq 'both')) {
    $MessageSniffer::conf{'neg_decisive'} = '-'.$MessageSniffer::conf{'position'}.'- ';
  } else {
    $MessageSniffer::conf{'neg_decisive'} = '~'.$MessageSniffer::conf{'position'}.'~ ';
  }
}
...
## analyze result
  ...
  if ($SNF_XCI_Return->{code} > 0) {
    ## is spam
    MailScanner::Log::InfoLog("$MODULE ".$MessageSniffer::conf{pos_decisive}."result is spam ($desc) for ".$message->{id});
    if ($MessageSniffer::conf{'putSpamHeader'}) {
      $global::MS->{mta}->AddHeaderToOriginal($message, $MessageSniffer::conf{'header'}, $MessageSniffer::conf{pos_decisive}."is spam ($desc)");
    }
    $message->{prefilterreport} .= ", MessageSniffer (".$MessageSniffer::conf{pos_decisive}."$desc)";
    return 1;
  } 
  MailScanner::Log::InfoLog("$MODULE ".$MessageSniffer::conf{neg_decisive}."result is not spam ($desc) for ".$message->{id});
  if ($MessageSniffer::conf{'putHamHeader'}) {
    $global::MS->{mta}->AddHeaderToOriginal($message, $MessageSniffer::conf{'header'}, $MessageSniffer::conf{neg_decisive}."is not spam ($desc)");
  }
  #$message->{prefilterreport} .= ", MessageSniffer (".$MessageSniffer::conf{neg_decisive}."$desc)";
  return 0;
}
