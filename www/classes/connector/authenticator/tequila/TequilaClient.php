...
class TequilaClient
{
    ...
    function TequilaClient($sServer = '', $sSessionsDirectory = '', $iTimeout = NULL)
    {
        ...
        if (empty($sSessionsDirectory)) {
            $sSessionsDirectory = $aEtcConfig['sSessionsDirectory'];
        }
    ...
    function SetAuthenticationMethod($iAuthenticationMethod)
    { // Nothing, obsolete.
    }
    ...
    function RemoveWantedGroups($aWantedGroups)
    {
        foreach ($this->aWantedGroups as $aWantedGroup) {
            if (in_array($aWantedGroup, $aWantedGroups)) {
                unset($this->aWantedGroups[array_search(
                    $aWantedGroup,
                    $this->aWantedGroups
                )]);
            }
    ...
    function SetCookieLife($iTimeout)
    { // Obsolete
    ...
    function CreateSession($attributes)
    {
        ...
        $sKey = $this->sKey;
        if (empty($sKey)) {
            return ($this->Error(ERROR_NO_KEY));
        }

        $sFile = $sSessionsDirectory . DIRECTORY_SEPARATOR . $sKey;
        if (file_exists($sFile)) {
            return ($this->Error(ERROR_SESSION_FILE_EXISTS));
        }

        $rHandle = @fopen($sFile, 'wb');
        if (!$rHandle) {
            return ($this->Error(ERROR_CREATE_FILE));
        }
        $sAttributes = "";
        ...
        @fclose($rHandle);
        if (!filesize($sFile)) {
            return ($this->Error(ERROR_CREATE_SESSION_FILE));
        }
        return (TRUE);
    }
