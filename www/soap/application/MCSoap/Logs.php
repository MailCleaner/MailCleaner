...
class MCSoap_Logs
{
    ...
    static public function Logs_ExtractLog($params)
    {
        ...
        if (is_array($levels[0])) {
            $res['log_stage1'] = $levels[0];
        }
        if (is_array($levels[1])) {
            $res['log_stage2'] = $levels[1];
        }
        if (is_array($levels[2])) {
            $res['log_engine'] = $levels[2];
        }
        if (is_array($levels[3])) {
            $res['log_stage4'] = $levels[3];
        }
        if (is_array($levels[4])) {
            $res['log_spamhandler'] = $levels[4];
        }
        if (is_array($levels[5])) {
            $res['log_finalstage4'] = $levels[5];
        }
